...
class RoutesConstructor extends Component {
    ...
    const DragHandle = SortableHandle(({ title }) => (
      ...
      SortableItem = SortableElement(({ item }) => (
        <div className="point-list__item">
          ...
          <span
            className="remove"
            onClick={(e) => this.removeMarker(item.id, e)}
          />
        </div>
      )),
      SortableList = SortableContainer(({ points }) => (
  ...
  onSortEnd = ({ oldIndex, newIndex }) => {
    this.setState({
      points: arrayMove(this.state.points, oldIndex, newIndex).map(
        (item, i) => {
          item.index = i;
  ...
  handleWinResize() {
    ...
    this.__resizeTimeout = setTimeout(() => {
      ...
      pointList.style.height =
        Node.clientHeight - cont.clientHeight + pointList.clientHeight + "px";
    }, 50);
  }
