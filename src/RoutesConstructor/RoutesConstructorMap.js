...
class RoutesConstructorMap extends Component{
	...
	render(){
		...
		this.__Markers.forEach((item,i)=>indexedMarkers[item.id]=this.__Markers[i]);
		points.forEach((item,i)=>indexedPoints[item.id]=points[i]);

		this.__Markers=this.__Markers.filter(item=>{
		...
		points.filter(item=>!indexedMarkers[item.id]).forEach(item=>{
			item.marker=new window.ymaps.Placemark(item.position,{
	...
	renderPath(){
		...
		if(this.__Path)
			this.__Map.geoObjects.remove(this.__Path);
	...
	getCenter(){
		return this.__state===2?this.__Map.getCenter():[0,0];
	}
	...
	__init(){
		...
		window.ymaps.ready(function(){
			window.__Map=That.__Map=new window.ymaps.Map(ReactDOM.findDOMNode(That.refs.routesConstructorMap),{
				center:That.getCenter(),
				zoom:10
			});
			That.__state=2;

			window.ymaps.geocode('Москва').then(function(res){
				That.__Map.setCenter(
